...
| Model | Pros | Cons |
|---------|---------|---------|
| BERT | • Bi-directional<br>• Captures contextual relationships between words<br>• Lower latency | • Requires GPU costs<br>• Doesn’t handle a larger input text<br>• Considered more of an architecture that needs tuning than a service |
| Embeddings API | • Larger token window<br>• Ready to use service<br>• | • Risk of PII data being input<br>• API costs could be larger |
...
| DB | Pros | Cons |
|---------|---------|---------|
| SQL Lite | • Supports vector stoage<br>• Simply to set up | • Not scalable at all |
| **Chroma** | • Integrates well with AI code packages such as LangChain<br>• | • Not scalable as well |
|  |  |  |
